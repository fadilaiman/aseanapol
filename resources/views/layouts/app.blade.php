...
<body class="font-sans bg-background dark:bg-dark text-gray-800 dark:text-gray-200 antialiased transition-colors duration-300{{ \Illuminate\Support\Facades\Route::currentRouteName() === 'landing' ? ' notranslate' : '' }}">
    ...
    {{-- Sticky Share Widget (left side) --}}
    <div class="fixed left-4 top-1/2 -translate-y-1/2 z-50 flex flex-col items-center gap-1.5" id="share-widget">
        {{-- Toggle button --}}
        <button onclick="toggleShareWidget()"
            class="group flex flex-col items-center gap-1 w-10 bg-white dark:bg-dark-card shadow-md rounded-xl px-1.5 py-2 hover:bg-primary hover:text-white dark:hover:bg-primary transition-all duration-200 hover:scale-105 hover:shadow-lg text-primary dark:text-accent"
            ...
            <span class="text-[8px] font-bold uppercase tracking-widest leading-none">Share</span>
        </button>

        {{-- Share options (hidden by default) --}}
        <div id="share-options" class="hidden flex-col items-center gap-1.5">
            {{-- Facebook --}}
            <button onclick="shareToSocial('facebook')" title="Share on Facebook"
            ...
            {{-- LinkedIn --}}
            <button onclick="shareToSocial('linkedin')" title="Share on LinkedIn"
            ...
            {{-- X (Twitter) --}}
            <button onclick="shareToSocial('twitter')" title="Share on X"
            ...
            {{-- WhatsApp --}}
            <button onclick="shareToSocial('whatsapp')" title="Share on WhatsApp"
                class="w-9 h-9 flex items-center justify-center rounded-xl bg-white dark:bg-dark-card shadow-md hover:bg-[#25D366] text-[#25D366] hover:text-white transition-all duration-200 hover:scale-110 hover:shadow-lg">
    ...
    function toggleShareWidget() {
        ...
        opts.classList.toggle('flex', isHidden);
    }
    ...
    function shareToSocial(platform) {
        ...
        const content = path || 'home';

        const params = {
            facebook:  `utm_source=facebook&utm_medium=social&utm_campaign=website_share&utm_content=${content}`,
            linkedin:  `utm_source=linkedin&utm_medium=social&utm_campaign=website_share&utm_content=${content}`,
            twitter:   `utm_source=twitter&utm_medium=social&utm_campaign=website_share&utm_content=${content}`,
            whatsapp:  `utm_source=whatsapp&utm_medium=social&utm_campaign=website_share&utm_content=${content}`,
        };

        const utmUrl = rawUrl + '?' + params[platform];
        ...
        window.open(shareUrls[platform], '_blank', 'width=600,height=450,noopener,noreferrer');
    }
    ...
    <div class="social-sidebar fixed right-4 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-2">
        ...
        <a href="https://twitter.com/ASEANAPOL1981" target="_blank" rel="noopener" aria-label="Twitter / X"
           class="w-9 h-9 flex items-center justify-center rounded-xl bg-white shadow-md hover:bg-black text-black hover:text-white transition-all duration-200 hover:scale-110 hover:shadow-lg">
            <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
        </a>
        <a href="https://www.linkedin.com/in/aseanapol-secretariat-344498ab" target="_blank" rel="noopener" aria-label="LinkedIn"
           class="w-9 h-9 flex items-center justify-center rounded-xl bg-white shadow-md hover:bg-[#0A66C2] text-[#0A66C2] hover:text-white transition-all duration-200 hover:scale-110 hover:shadow-lg">
            <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
        </a>
        <a href="https://www.instagram.com/aseanapol" target="_blank" rel="noopener" aria-label="Instagram"
           class="w-9 h-9 flex items-center justify-center rounded-xl bg-white shadow-md hover:bg-[#E1306C] text-[#E1306C] hover:text-white transition-all duration-200 hover:scale-110 hover:shadow-lg">
