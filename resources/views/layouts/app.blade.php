...
<head>
    <!-- Google tag (gtag.js) -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-3Q6MSP87BY"></script>
    <script>
      window.dataLayer = window.dataLayer || [];
      function gtag(){dataLayer.push(arguments);}
      gtag('js', new Date());
      gtag('config', 'G-3Q6MSP87BY');
    </script>
    <meta charset="UTF-8">
...
<body class="font-sans bg-background dark:bg-dark text-gray-800 dark:text-gray-200 antialiased transition-colors duration-300{{ \Illuminate\Support\Facades\Route::currentRouteName() === 'landing' ? ' notranslate' : '' }}">
    ...
    {{-- Right sidebar: collapsible Follow icons + single Share button with flyout --}}
    <div class="fixed right-0 top-1/2 -translate-y-1/2 z-50 flex flex-row items-center" id="right-sidebar">

        {{-- Collapse toggle tab --}}
        <button onclick="toggleSidebar()" id="sidebar-toggle"
            class="flex items-center justify-center w-5 h-14 bg-white/80 dark:bg-dark-card/80 shadow-md rounded-l-lg hover:bg-primary hover:text-white dark:hover:bg-primary text-primary dark:text-accent transition-all duration-200"
            aria-label="Toggle sidebar" title="Show / Hide">
            <svg id="sidebar-chevron" class="w-3 h-3 fill-current transition-transform duration-300" viewBox="0 0 24 24">
                <path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/>
            </svg>
        </button>

        {{-- Icons panel --}}
        <div id="sidebar-panel" class="flex flex-col items-center gap-2 pr-3 pl-1 py-3">

            {{-- Follow icons (no label) --}}
            <a href="https://www.facebook.com/ASEANAPOL" target="_blank" rel="noopener" aria-label="Facebook"
               class="w-9 h-9 flex items-center justify-center rounded-xl bg-white dark:bg-dark-card shadow-md hover:bg-[#1877F2] text-[#1877F2] hover:text-white transition-all duration-200 hover:scale-110 hover:shadow-lg">
                <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
            </a>
            <a href="https://twitter.com/ASEANAPOL1981" target="_blank" rel="noopener" aria-label="X / Twitter"
               class="w-9 h-9 flex items-center justify-center rounded-xl bg-white dark:bg-dark-card shadow-md hover:bg-black text-black dark:text-white hover:text-white transition-all duration-200 hover:scale-110 hover:shadow-lg">
                <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            </a>
            <a href="https://www.linkedin.com/in/aseanapol-secretariat-344498ab" target="_blank" rel="noopener" aria-label="LinkedIn"
               class="w-9 h-9 flex items-center justify-center rounded-xl bg-white dark:bg-dark-card shadow-md hover:bg-[#0A66C2] text-[#0A66C2] hover:text-white transition-all duration-200 hover:scale-110 hover:shadow-lg">
                <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
            </a>
            <a href="https://www.instagram.com/aseanapol" target="_blank" rel="noopener" aria-label="Instagram"
               class="w-9 h-9 flex items-center justify-center rounded-xl bg-white dark:bg-dark-card shadow-md hover:bg-[#E1306C] text-[#E1306C] hover:text-white transition-all duration-200 hover:scale-110 hover:shadow-lg">
                <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/></svg>
            </a>

            {{-- Divider --}}
            <div class="w-6 border-t border-gray-200 dark:border-gray-600 my-0.5"></div>

            {{-- Single Share button with flyout (opens LEFT as horizontal row) --}}
            <div class="relative" id="share-btn-wrap">
                <button onclick="toggleShareFlyout(event)" title="Share this page"
                    class="w-9 h-9 flex items-center justify-center rounded-xl bg-white dark:bg-dark-card shadow-md hover:bg-primary text-primary hover:text-white dark:text-accent transition-all duration-200 hover:scale-110 hover:shadow-lg">
                    <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92s2.92-1.31 2.92-2.92-1.31-2.92-2.92-2.92z"/></svg>
                </button>

                {{-- Flyout: hidden by default via inline style, opens horizontally to the LEFT --}}
                <div id="share-flyout"
                    style="display:none; position:absolute; right:calc(100% + 10px); top:50%; transform:translateY(-50%); flex-direction:row; align-items:center; gap:8px; background:#fff; box-shadow:0 4px 24px rgba(0,0,0,.15); border-radius:16px; padding:6px 12px; border:1px solid #f0f0f0; white-space:nowrap; z-index:60;">
                    <button onclick="shareToSocial('facebook')" title="Facebook"
                        style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;border-radius:10px;border:none;background:transparent;cursor:pointer;color:#1877F2;"
                        onmouseenter="this.style.background='#1877F2';this.style.color='#fff';" onmouseleave="this.style.background='transparent';this.style.color='#1877F2';">
                        <svg style="width:16px;height:16px;fill:currentColor;" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                    </button>
                    <button onclick="shareToSocial('linkedin')" title="LinkedIn"
                        style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;border-radius:10px;border:none;background:transparent;cursor:pointer;color:#0A66C2;"
                        onmouseenter="this.style.background='#0A66C2';this.style.color='#fff';" onmouseleave="this.style.background='transparent';this.style.color='#0A66C2';">
                        <svg style="width:16px;height:16px;fill:currentColor;" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                    </button>
                    <button onclick="shareToSocial('twitter')" title="X / Twitter"
                        style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;border-radius:10px;border:none;background:transparent;cursor:pointer;color:#000;"
                        onmouseenter="this.style.background='#000';this.style.color='#fff';" onmouseleave="this.style.background='transparent';this.style.color='#000';">
                    ...
                    <button onclick="shareToSocial('whatsapp')" title="WhatsApp"
                        style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;border-radius:10px;border:none;background:transparent;cursor:pointer;color:#25D366;"
                        onmouseenter="this.style.background='#25D366';this.style.color='#fff';" onmouseleave="this.style.background='transparent';this.style.color='#25D366';">
    ...
    <script>
    (function() {
        if (localStorage.getItem('sidebarCollapsed') === 'true') applySidebarState(true);
    })();
    ...
    function toggleSidebar() {
        const next = !(localStorage.getItem('sidebarCollapsed') === 'true');
        localStorage.setItem('sidebarCollapsed', next);
        applySidebarState(next);
        closeShareFlyout();
    }

    function toggleShareFlyout(e) {
        e.stopPropagation();
        const flyout = document.getElementById('share-flyout');
        flyout.style.display = (flyout.style.display === 'flex') ? 'none' : 'flex';
    }

    function closeShareFlyout() {
        const flyout = document.getElementById('share-flyout');
        if (flyout) flyout.style.display = 'none';
    }

    document.addEventListener('click', function(e) {
        if (!document.getElementById('share-btn-wrap').contains(e.target)) closeShareFlyout();
    });
    ...
    function shareToSocial(platform) {
        ...
        const content = (window.location.pathname.replace(/\//g, '_').replace(/^_/, '')) || 'home';
        const utmUrl  = rawUrl + '?utm_source=' + platform + '&utm_medium=social&utm_campaign=website_share&utm_content=' + content;
        const urls = {
            ...
            twitter:  'https://twitter.com/intent/tweet?url=' + encodeURIComponent(utmUrl) + '&text=' + encodeURIComponent(title),
