...
<body class="font-sans bg-background dark:bg-dark text-gray-800 dark:text-gray-200 antialiased transition-colors duration-300{{ \Illuminate\Support\Facades\Route::currentRouteName() === 'landing' ? ' notranslate' : '' }}">
    ...
    {{-- Right sidebar: collapsible Follow + Share --}}
    <div class="fixed right-0 top-1/2 -translate-y-1/2 z-50 flex flex-row items-center gap-0" id="right-sidebar">

        {{-- Collapse toggle tab --}}
        <button onclick="toggleSidebar()" id="sidebar-toggle"
            class="flex items-center justify-center w-5 h-16 bg-white/80 dark:bg-dark-card/80 shadow-md rounded-l-lg hover:bg-primary hover:text-white dark:hover:bg-primary text-primary dark:text-accent transition-all duration-200"
        ...
        {{-- Icons panel --}}
        <div id="sidebar-panel" class="flex flex-col items-center gap-2 pr-3 pl-1 py-3 transition-all duration-300">

            {{-- FOLLOW section label --}}
            <span class="text-[7px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500 leading-none">Follow</span>
            ...
            {{-- SHARE section label --}}
            <span class="text-[7px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500 leading-none">Share</span>
    ...
    (function() {
        ...
        if (collapsed) applySidebarState(true);
    })();
